...
public class CrystalFinanceController(IMySqlDataService sqlCrud, ILogger<CrystalFinanceController> logger) : ControllerBase
{
    ...
    public async Task<ActionResult<PagedResult<TransactionModel>>> Get([FromQuery] int pageNumber = 1, [FromQuery] int pageSize = 20)
    {
    ...
    public async Task<ActionResult<TransactionModel?>> Get(int id)
    {
        ...
        catch (Exception ex)
        {
            ...
            return StatusCode(500, $"An error occurred while retrieving the transaction. ID: {id}");
        }
    }
    ...
    public async Task<IActionResult> Put(int id, [FromBody] TransactionModel value)
    {
        ...
        try
        {
            ...
            return Ok(new { message = $"Transaction updated successfully. ID: {id}" });
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error updating transaction. ID: {TransactionId}", id);
            return StatusCode(500, $"An error occurred while updating the transaction. ID: {id}");
        }
    }
    ...
    public async Task<IActionResult> Delete(int id)
    {
        ...
        try
        {
            ...
            return Ok(new { message = $"Transaction deleted successfully. ID: {id}" });
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error deleting transaction. ID: {TransactionId}", id);
            return StatusCode(500, $"An error occurred while deleting the transaction. ID: {id}");
        }
    }
}
